...
plot.prior.var<-function(beta,sigma,percent = 99){
  ...
  p<- p+geom_text(aes(beta,y1,label = paste("beta" , "^",1,1:n.studies,"*", "'' %+-% '' ","*", round(multiplier,3),"*","sigma","^",1:4, sep = "")),parse = TRUE,nudge_y = -0.03)
  p<- p+geom_text(aes(rep(0,4),y2,label = paste("beta" , "^",1:n.studies,"*", "'' %+-% '' ","*", round(multiplier,3),"*","sigma","^",1:4, sep = "")),parse = TRUE,nudge_y = -0.03)
  p<- p + xlab(NULL) + ylab(NULL)
  p<-p+ ggtitle("Prior variance credibility and observed variance confidence intervals")
  p
}
```
